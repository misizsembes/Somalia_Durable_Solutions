...
#numeric <- prep_stat_tests(rdss_data, "all",90, "has_your_household_always_lived_in_the_settlement_you_are_currently_living_in")
...
#REMOVE DEMOGRAPHICS FOR AGGREGATION
...
#DATA AGGREGATION
#rdss_aggregated <- loop_over_dataset(rdss_data_analyze,"what_is_the_name_of_the_village_settlement_not_the_idp_site_name","which_of_the_following_languages_would_you_prefer_to_receive_information_in","all", "has_your_household_always_lived_in_the_settlement_you_are_currently_living_in")
...
###################################################################################
#NEED FOR CHI2:
#grouping_var == GROUP CATEGORIES FOR TESTING == "displaced" 
#first_chi2_var === #FIRST BINARY VARIABLE TO CALCULATE CHI2 == "hoh_gender_female"   
#last_chi2_var == #LAST BINARY VARIABLE TO CALCULATE CHI2 == "q.6.3_economic_opportunity_type_new_location_livestock_production_for_personal_use"  
###################################################################################
...
apple <- chi2_go(cateez[[5]] , "has_your_household_always_lived_in_the_settlement_you_are_currently_living_in" )



###PREPARE DATA FOR STATISTICAL TESTS
stat_test_preps <- list()
for( j in grep("what_is_the_name_of_the_village_settlement_not_the_idp_site_name",colnames(rdss_data)): grep("which_of_the_following_languages_would_you_prefer_to_receive_information_in",colnames(rdss_data)) ){
  stat_test_preps[[j]] <- prep_stat_tests(rdss_data, "all",j, "has_your_household_always_lived_in_the_settlement_you_are_currently_living_in")
}
stat_test_preps[sapply(stat_test_preps, is.null)] <- NULL

###PARSE CATEGORICAL AND NUMERIC
select_var_type <- function(dataa, type_var, agg_var, grouping_var){
  ...
  cateez[sapply(cateez, is.null)] <- NULL
  return(cateez)
}

############STATISTICAL TESTS###############
#SEPARATED CATE & NUMERIC DATASETS
cateez <- select_var_type(stat_test_preps, "categorical", "global_agg","has_your_household_always_lived_in_the_settlement_you_are_currently_living_in")
#numerik <- select_var_type(stat_test_preps, "numeric","global_agg","has_your_household_always_lived_in_the_settlement_you_are_currently_living_in")
###CHI2 LOOP###
chi2_results_go <- list()
for(g in 1:length(cateez)){
chi2_results_go[[g]] <- chi2_go(cateez[[g]] , "has_your_household_always_lived_in_the_settlement_you_are_currently_living_in" )
}
...
write.csv(chi2_summary,"chi2_summary.csv")

###################################T-TEST##########################################
#NEED FOR TTEST:
#grouping_var == GROUP CATEGORIES FOR TESTING == "displaced" 
#first_ttestvar == FIRST VARIABLES FOR TTEST == "q.1.4_head_of_hh_age"
#last_ttestvar == LAST VARIABLES FOR TTEST ==  "q.1.5_youth_age"
############################################################################################
#REMOVE COLUMNS THAT ARE COMPRISED OF ALL NAs#REMOVE IF COLUMNS ARE ALL 1 AND BLANK ROWS
ttest_go <- function(dataa,grouping_var,first_ttestvar,last_ttestvar){
  for_ttest <- dataa %>% dplyr::filter(!is.na(UQ(as.name(grouping_var)))) %>%
    dplyr::select_if(~ length(unique(.)) > 1) 
  #FIRST VARIABLE TO CONDUCT CHI2
  first_ttestvar_index <- grep(first_ttestvar,colnames(for_ttest))
  last_ttestvar_index <- grep(last_ttestvar,colnames(for_ttest))
  #GROUPING COLUMN INDEX
  ttest_grouping_var <- grep(grouping_var, colnames(for_ttest)) 
  #TTEST LOOP
  ttest_list <- list()
  for(i in first_ttestvar_index: last_ttestvar_index){
    for_ttest[,i] <- as.numeric(for_ttest[,i])
    ...
    ttest_list[[i]] <- data.frame(question_name,group1_sample_size,group2_sample_size,group1_name,group2_name,group1_mean,group2_mean,group1_sd,group2_sd, ttest_stat,ttest_pval,lower_confid_interval,upper_confid_interval)
  }
  ttest_list[sapply(ttest_list, is.null)] <- NULL
  ttest_summary  <- do.call(rbind.data.frame, ttest_list)
  #CREATE COLUMN OF SIGNIFICANT RESULTS AND IDENTIFY THE GROUP WITH THE LARGER MEAN
